...
class Timer:
    ...
    def create_EnemyTwo(self):
        if (self.time_two >= 6 and self.boss_live == False):
    ...
    def create_EnemyFour(self):
        if (self.time_four >= 6 and self.boss_live == False):
            new_EnemyFour = EnemyFour()
            EnemyFour_L.append(new_EnemyFour)
            self.time_four = 0.0
...
class EnemyThree:
    ...
    def draw(self):
        self.image.clip_draw(self.frame * 135, 0, 135, 180, self.x, self.y)
