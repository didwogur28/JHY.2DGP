...
class Timer:
    ...
    def create_EnemyOne(self):
        if (self.time_one >= 1 and self.boss_live == False):
            new_EnemyOne = EnemyOne()
            EnemyOne_L.append(new_EnemyOne)
            self.time_one = 0.0

    def create_EnemyTwo(self):
        if (self.time_two >= 3 and self.boss_live == False):
...
class EnemyOne:
    ...
    def __init__(self):
        self.x, self.y = random.randint(80, 750), random.randint(800, 900)
        self.xdir = 0
        self.ydir = 0
        self.LIFE = True
    ...
    def handle_up_stand(self):
        self.stand_frames += 1
        if self.stand_frames == 60:
            self.state = self.UP_RUN
            self.run_frames = 0

    handle_state = {DOWN_RUN: handle_down_run, UP_RUN:handle_up_run, DOWN_STAND:handle_down_stand, UP_STAND: handle_up_stand }
...
class EnemyTwo:
    ...
    def __init__(self):
        self.x, self.y = random.randint(400, 800), random.randint(1000, 1100)
        self.xdir = 0
        self.ydir = 0
        self.LIFE = True
        ...
        if EnemyTwo.image == None:
            EnemyTwo.image = load_image('object/Enemy/Enemy1.png')
    ...
    def handle_down_run(self):
        ...
        if self.x < 100:
            self.state = self.UP_RUN
        if self.run_frames == 200:
            self.state = self.DOWN_STAND
            self.stand_frames = 0
    def handle_down_stand(self):
        self.stand_frames += 1
        if self.stand_frames == 50:
            self.stand_frames = self.DOWN_RUN
            self.run_frames = 0
    ...
    def draw(self):
        self.image.clip_draw(self.frame * 83, 0, 73, 120, self.x, self.y)
...
class EnemyThree:
    ...
    def draw(self):
        self.image.clip_draw(self.frame * 135, 0, 135, 160, self.x, self.y)
...
# 현재 상태 : 게임 시작 됌, 적 이상한데서 나옴,, 필살기가 자동으로 실행
#             최적화가 안됌
